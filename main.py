...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.incorrect_password_label = tk.Label(self,
                                                                        text='',
                                                                        font=('orbitron',13),
                                                                        fg='white',
                                                                        bg='#33334d',
    ...
    def check_password(self):
        c.execute("SELECT * FROM users WHERE username=? AND password=?", (self.my_username.get(), self.my_password.get()))
        result = c.fetchone()
    ...
    def register(self):
        username = self.my_username.get()
        password = self.my_password.get()

        if register_user(username, password):
            self.my_password.set('')
            self.incorrect_password_label['text'] = 'Registration successful!'
        else:
            self.incorrect_password_label['text'] = 'Registration failed! Try another username.'
